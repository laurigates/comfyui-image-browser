...
| Flat view | On a sandboxed tab, the toolbar **≣** toggle folds the current folder's whole subtree into one grid (newest-first), each card labelled above the thumbnail with its relative subpath. Tapping a subpath label drops back to folder view at that directory. Delete/rename/move/rating and multi-select all operate on each card's real (nested) subfolder. Search matches `subpath/name`. Hidden on the browse…/path tab. Preference persists (localStorage `view`); a very large tree shows a "truncated" toast. |
